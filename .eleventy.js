...
module.exports = function (eleventyConfig) {
  ...
  // add handler to convert "2024-02" -> "02"
  eleventyConfig.addFilter("getweek", (fileslug) => {
    let [year, week] = fileslug.split("-");
    return week;
  });
  ...
  // pad number with 0s to be length len
  eleventyConfig.addFilter("padzeros", (num, len) =>
    String(num).padStart(len, "0")
  );
  // turn string into array of characters
  eleventyConfig.addFilter("str2arr", (s) => s.toString().split(""));
  ...
  eleventyConfig.addFilter("range", (to) => [...Array(to).keys()]);
  // add handler to get key from object
  eleventyConfig.addFilter("get", (obj, key) => obj[key]);
  // add handler to get index from array
  eleventyConfig.addFilter("getidx", (arr, idx) => arr[idx]);
  ...
  // add handler for "equals"
  eleventyConfig.addFilter("eq", (c1, c2) => c1 == c2);
  // add handler for "or"
  eleventyConfig.addFilter("or", (c1, c2) => c1 || c2);
  // add handler for "and"
  eleventyConfig.addFilter("and", (c1, c2) => c1 && c2);
  // add handler for "not"
  eleventyConfig.addFilter("not", (c1) => !c1);

  // add helper handler to view data as json
  eleventyConfig.addFilter("json", (obj) => JSON.stringify(obj));
  // add helper handler to view type of data
  eleventyConfig.addFilter("typeof", (obj) => typeof obj);
