...
```bash
curl -s "https://weeknotes.alifeee.net/" | pcregrep -o1 "([0-9,]*) words" | sed 's/,//g' | awk '{tot += $0} END {print tot}'
```

Running this suggests that I have written 26,173 words. Having measured all of my [academic writing](https://alifeee.net/durham_physics/) in pages, not words, I don't really have a conception of how much this is. Is it a lot? Maybe.
